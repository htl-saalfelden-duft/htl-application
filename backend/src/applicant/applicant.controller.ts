...
export class ApplicantController {
  ...
  async sokratesCsv(
    ...
    return await this.applicantService.exportSokratesCSV({ statusKey }).then(
      async (fileName) =>
        await this.applicantService.getExportedCSV(fileName).then((csvData) => {
  ...
  btsCsv(
    @Response() res: ExpressResponse,
    @Query('statusKey') statusKey: string,
    @Query('schoolClass') schoolClass: string,
    @Query('search') search: string,
  ): Observable<ExpressResponse> {
    let where: Prisma.ApplicantWhereInput = {};

    this.buildWhereClause({ statusKey, schoolClass, search }, where);

    return this.applicantService.exportBtsCSV(where).pipe(
      mergeMap((fileName) => this.applicantService.getExportedCSV(fileName)),
      map((csvData) => {
        res.set('Content-Type', 'text/csv');
  ...
  @Post()
  async create(
    @Body() applicantData: Prisma.ApplicantCreateInput,
  ): Promise<Applicant> {
  ...
  signIn(@Body() signInDto: SignInDto): Observable<TokenDto> {
    return this.applicantService
      .checkCredentials(signInDto)
      .pipe(map((id) => this.authService.createToken(id, 'applicant')));
  }

  @Public()
  @Post('register')
  registerApplicant(
    @Body() dto: Prisma.ApplicantCreateInput,
  ): Observable<Applicant> {
    return from(this.applicantService.create(dto)).pipe(
      mergeMap((applicant) =>
        this.emailConfirmationService
          .sendVerificationLink(applicant.email)
          .pipe(
            map(() => {
  ...
  confirm(@Body() confirmationData: ConfirmEmailDto): Observable<unknown> {
    return from(
      this.emailConfirmationService.decodeConfirmationToken(
  ...
  private buildWhereClause(params, where) {
    ...
    this.injectSearchParams(search, where)
  }
